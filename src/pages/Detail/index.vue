        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(ValueList, index) in skuInfo.skuSaleAttrValueList"
                :key="ValueList.id"
              >
                <dt class="title">{{ ValueList.saleAttrName }}</dt>
...
export default {
  ...
  mounted() {
    //派发action获取产品详情
    this.$store.dispatch("getItemInfo", this.$route.params.skuid);
  },
  computed: {
  ...
  methods: {
    ...
    async addShopCar() {
      try {
        ...
        //路由跳转
        //顺便将产品信息带给下一级路由
        //简单数据skuNum通过query形式传递过去
        //产品信息数据skuInfo 通过会话存储传递：非持久化，页面结束数据消失
        //无论是本地存储、会话存储一般存储的为字符串
